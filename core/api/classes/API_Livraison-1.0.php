...
class API_Livraison {
	...
	public function liste_pays($id_langues){
		$q = <<<SQL
...
WHERE fp.id_langues = {$id_langues}
SQL;
...
SQL;
	...
	public function forfait ($id_langues, $montant, $id_pays, $id_boutiques = 0) {
		$q = <<<SQL
...
SQL;
	...
	function check($id_langues, $id_boutiques = 0){
		$data = $this->api->post;
		$id_pays = $data['commande']['livraison_pays'];
		$q = <<<SQL
SELECT methode from dt_frais_port
WHERE id_langues = {$id_langues} AND id_pays = {$id_pays} AND id_boutiques = $id_boutiques
SQL;
		$res = $this->sql->query($q);
		$row = $this->sql->fetch($res);
		if (!$row) {
