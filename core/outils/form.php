...
class Form {
  ...
  public function next_or_previous() {
	...
	public function first_or_next_or_previous() {
		if (isset($_SERVER['HTTP_REFERER']) and $_SERVER['HTTP_REFERER'] == "http://".$_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI']) {
	  		$this->next_or_previous();
		}
		else {
			$this->step(1);
		}
  ...
  public function html($html) {
	...
	public function select($params) {
		...
		foreach ($options as $cle => $valeur) {
			...
			if (isset($params['enable']) and !preg_match($params['enable'], $valeur)) {
    			$params['field'] .= ' disabled="disabled"';
			}
			if (isset($params['disable']) and preg_match($params['disable'], $valeur)) {
    			$params['field'] .= ' disabled="disabled"';
			}
